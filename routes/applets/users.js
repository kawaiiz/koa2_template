...
router.post('/login', async (ctx) => {
  ...
  try {
    ...
    let user = await query(userAppSql.check, [mobile])
    ...
    if (type === 'password') {
      ...
      let redisMsg = await redis.get(`${mobile}appLogin`)
      if (redisMsg === msg) {
        // 删除缓存中的短信
        redis.del(`${mobile}appLogin`);
        _OK()
      } else {
        throw '验证码错误'
      }
...
router.post('/register', async (ctx) => {
  ...
  try {
    ...
    let id = uuidv1()
    //插入数据库
    await query(userAppSql.insert, [id, username, await util.enbcrypt(password), mobile])
    result = {
      status: 1,
      msg: '添加用户成功'
    }
...
router.post('/updata', async (ctx) => {
  ...
  try {
    ...
    await query(userAppSql.update(ctx.request.body, ctx.state.id))
    result = {
      status: 1,
      msg: '修改成功'
    }
...
router.post('/resetPassword', async (ctx) => {
  ...
  try {
    ...
    // 短信验证
    let redisMsg = await redis.get(`${mobile}appSetPassword`)
    if (redisMsg !== msg) throw '短信不正确';
    redis.del(`${mobile}appSetPassword`);

    password = await util.enbcrypt(password)
    await query(userAppSql.resetPassword, [password, ctx.state.id])
    result = {
      status: 1,
      msg: '修改成功'
    }
 ...
 * @desc 修改手机号码
 * @access 接口公开
 * @parameter  mobile ，msg
 * */
let resetMobileRuler = {
  mobile: [{type: 'string', required: true, pattern: /^1\d{10}/, message: '请查看新手机号格式是否正确'}],
  msg: [{type: 'string', required: true, message: '短信不能为空', min: 1}]
}
let resetMobileValidator = new schema(resetMobileRuler);
router.post('/resetMobile', async (ctx) => {
  let result = {}
  try {
    // 字段验证
    await resetMobileValidator.validate(ctx.request.body)
    console.log(ctx.state)
    let oldMobile = ctx.state.mobile
    let {mobile, msg} = ctx.request.body
    // 短信验证
    let redisMsg = await redis.get(`${oldMobile}appSetMobile`)
    console.log(`${oldMobile}appSetMobile : ${redisMsg}`)
    if (redisMsg !== msg) throw '短信不正确';
    redis.del(`${oldMobile}appSetMobile`);
    await query(userAppSql.resetMobile, [mobile, ctx.state.id])
    result = {
      status: 1,
      msg: '修改成功'
    }
